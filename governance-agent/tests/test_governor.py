...
from governor import (AUTHORIZE, ESCALATE, WITHHOLD, Pipeline, evaluate, initial_policy, initial_state,  # noqa: E402
                      verify_chain)
...
class GovernorTests(unittest.TestCase):
    def test_evaluate_does_not_modify_state(self):
        s = initial_state()
        d = digest(s)
        evaluate(initial_policy(), s, {"action_type": "spend", "target": "x", "parameters": {"amount_cents": 1}})
    ...
    def test_same_input_same_verdict(self):
        ...
        a, b = evaluate(initial_policy(), initial_state(), p), evaluate(initial_policy(), initial_state(), p)
        self.assertEqual((a["verdict"], a["reasons"]), (b["verdict"], b["reasons"]))

    def test_unknown_action_withheld(self):
        r = evaluate(initial_policy(), initial_state(), {"action_type": "add_allowlist", "target": "x@y", "parameters": {}})
        self.assertEqual(r["verdict"], WITHHOLD)
        self.assertEqual(r["violated"], ["proposal_schema"])

    def test_withhold_leaves_real_state_unchanged(self):
        pipe = Pipeline(initial_state(), initial_policy())
        d = digest(pipe.state)
