...
from governor.state import initial_state
...
def evaluate(proposal):
    ...
    if t == "spend":
        remaining = _START["budget"]["ceiling_cents"] - _START["budget"]["spent_cents"]
    ...
    if t == "assign_role" and p.get("role") == "admin":
        return "WITHHOLD"
    if t == "send_message":
        return "AUTHORIZE" if target in _START["recipient_allowlist"] else "WITHHOLD"
    return "AUTHORIZE"
